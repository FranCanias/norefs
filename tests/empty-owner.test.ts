  ...
  it('reports a type alias that becomes empty', () => {
    const findings = analyzeSource(
      [
        ...
        '  verbose: boolean;',
        '};',
        'function useOptions(options: Options): number {',
        '  return 0;',
        '}',
        '',
      ].join('\n')
  ...
  it('stays silent when some member is used', () => {
    const findings = analyzeSource(
      [
        ...
        '  dead: number;',
        '}',
        'function usePartial(v: Partial1): number {',
        '  return v.used;',
        '}',
        '',
      ].join('\n')
  ...
  it('stays silent for an interface that extends another', () => {
    const findings = analyzeSource(
      [
        ...
        '  extra: string;',
        '}',
        'function useDerived(d: Derived): number {',
        '  return d.kept;',
        '}',
        '',
      ].join('\n')
  ...
  it('reports the property holding a type literal that loses every member', () => {
    const findings = analyzeSource(
      [
        ...
        '  };',
        '}',
        'function label(card: RecipeCard): string {',
        '  const { print } = card;',
