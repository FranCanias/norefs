...
function main(): void {
  const parse = () =>
    parseArgs({
      ...
      // The `--no-*` negations this help text documents. The option landed in
      // Node 22.4, which is why "engines" asks for it.
      allowNegative: true,
    });

  // An unknown flag or one missing its value is a usage error like any other,
  // and it used to arrive as a stack trace from inside node:util.
  let values: ReturnType<typeof parse>['values'];
  let positionals: string[];
  try {
    ({ values, positionals } = parse());
  } catch (error) {
    process.stderr.write(`error: ${firstLine(error)}\n`);
    process.exitCode = 2;
    return;
  }
  ...
  if (missingPaths.length > 0) {
    process.stderr.write(`error: no such path: ${missingPaths.join(', ')}\n`);
    process.exitCode = 2;
    return;
  }
