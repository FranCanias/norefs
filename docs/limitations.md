...
- a read of an array element — `cfg.rows[0].id`, `cfg.rows.map(r => r.id)`, `for (const r of cfg.rows)`, `cfg.rows.filter(…).map(…)` — counts on the element literals, and a name any one of them holds a read on is alive on all of them
...
- A top-level array binding is not read for its elements. `const cards = [{ id: 1, deadFlag: false }]` reports nothing: nesting reaches an array through the property that holds it, and this array has no holding property. Nest it under one — `const box = { cards: [ … ] }` — and the members answer.
