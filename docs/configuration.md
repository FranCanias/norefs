...
| `.storybook/main.*` | every file its `stories` globs match, written relative to `.storybook/` the way Storybook reads them — `'../stories/**/*.stories.@(js|jsx|mjs|ts|tsx)'`. Storybook loads every named export of a story, so a story is an entry point and its exports are never reported; it stays a harness file, so nothing it renders is production code on its account |
| a tool's own directory | the files a tool loads from it by name, when the tsconfig holds them: vitepress's `config.ts` and `theme/index.ts`, Storybook's `main.ts`, `preview.ts` and `manager.ts`. `include: [".vitepress/**/*"]` is how vue-tsc is pointed at a site, and the two files every site has are loaded by name and imported by nothing |
...
Negated globs (`'!packages/legacy'`) are honoured, and a declared package with no `tsconfig.json` is named on stderr rather than dropped quietly — nothing analyzes it, and a run that silently covers less than the workspace is a run whose findings mean less than they look like they mean. The root is a package of its own workspace whether the declaration names it (`.` beside `website`) or lists only what sits under it (`packages: [docs]` with the library at the root): its manifest answers for its own files either way, and its `tsconfig.json` is a project when it holds files. A root tsconfig that only references the others holds nothing, and is left out without a word:
