...
| --- | --- |
| `package.json` | `main`, `types`, `bin`, and `exports`; paths into the compiled output map back to source through the tsconfig `outDir` and `rootDir`. A subpath pattern — `"./*": "./dist/*.js"` — publishes every module it matches and names none of them, so it is matched against the files the run holds, harness files excepted. Naming none of them, it keeps a file alive without putting it on the shipping path |
